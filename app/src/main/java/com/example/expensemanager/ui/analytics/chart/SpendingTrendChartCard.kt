...
fun SpendingTrendChartCard(
    ...
    val totalSpending = points.sumOf { it.value }
    val highestPoint = points.maxByOrNull { it.value }
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = "Daily expenses over the last 14 days",
            ...
            if (points.all { it.value == 0.0 }) {
                ...
                Text(
                    text = "Total spending: ${formatCompactCurrency(totalSpending)}",
                    style = MaterialTheme.typography.bodyMedium
                )

                if (highestPoint != null) {
                    Text(
                        text = "Highest day: ${highestPoint.label} - ${formatCompactCurrency(highestPoint.value)}",
                        style = MaterialTheme.typography.bodyMedium
                    )
                }
            }
